...
class PollReader():
    ...
    def build_data_dict(self):
        ...
        for i in self.raw_data[1:]: #skip header row

            # split up the row by column
            seperated = i.strip().split(',') #separte by comma, add strip

            # map each part of the row to the correct column
            self.data_dict['month'].append(seperated[0])
            self.data_dict['date'].append(int(seperated[1])) #code error on this line
            self.data_dict['sample'].append(int(seperated[2].split(" ")[0])) #add split
    ...
    def highest_polling_candidate(self):
        ...
        if max_harris > max_trump:
            return f"Harris {(max_harris * 100):.1f}%" #times by 100 to find percent, use % to cut off output
